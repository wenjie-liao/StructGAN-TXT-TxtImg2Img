...
## 主函数获取SIoU
def main_SIoU(num_cases,img_path,p2p_or_p2pHD,results_path):
    if p2p_or_p2pHD == "p2p":
    ...
    for img_NO in range(num_cases):
        tar_imgpath = os.path.join(img_path,"test (%d)" %(img_NO+1)+"-targets")
        out_imgpath = os.path.join(img_path,"test (%d)" %(img_NO+1)+"-outputs")
        if os.path.exists(tar_imgpath) and os.path.exists(out_imgpath):
            result_dir = results_path + "\\SIoU\\test(%d)"%(img_NO+1) #创建结果文件夹
            if not os.path.exists(result_dir):
                os.makedirs(result_dir)
            
            inter_areas,union_areas = [],[]
            for m in range (div_H-1):
                for n in range (div_W-1):
                    real_img = tar_imgpath+"\\test (%d)"%(img_NO+1)+"-targets"+str(m)+str(n)+".png"
                    pre_img = out_imgpath+"\\test (%d)"%(img_NO+1)+"-outputs"+str(m)+str(n)+".png"
                    if os.path.exists(tar_imgpath) and os.path.exists(out_imgpath):
            ...
            SIoU = getIoU(inter_areas,union_areas)
            SIoUs.append(SIoU)
            
            # OUTPUT IOU
            SIoU_txtname = result_dir+"\\"+"SIoU(%d).txt"%(img_NO+1)
            with open(SIoU_txtname, "w+") as SIoU_txt:
                SIoU_txt.write("SIoU: " + str(SIoU) +"\n")
                for i,inter_area in enumerate(inter_areas):
                    SIoU_txt.write("inter_area: "+str(inter_area)+"\n")
                for j,union_area in enumerate(union_areas):
                    SIoU_txt.write("union_area: "+str(union_area)+"\n")
